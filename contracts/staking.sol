...
contract Staking is ERC20Capped, Ownable {
    ...
    ) ERC20("Staked Token", "sToken") ERC20Capped(1000000 * 1e18) {
        // 设定总供应上限为 100 万个代币，精确到 wei
        // _stakingToken - 质押代币类型的合约地址，拿到代币合约类型实例
        //IERC20(_stakingToken)：等于你告诉 Solidity：“请把位于这个地址的合约，当作一个 IERC20 接口的实例来对待”。
    ...
    function safeRewardTransfer(address to, uint256 amount) internal {
        // 查询当前质押合约在奖励代币合约中的余额
        uint256 rewardBal = IERC20(address(rewardToken)).balanceOf(
            address(this)
        );
